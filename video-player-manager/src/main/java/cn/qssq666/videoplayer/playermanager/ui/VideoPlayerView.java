...
public class VideoPlayerView extends ScalableTextureView
    ...
    private final Set<MediaPlayerWrapper.VideoStateListener> mVideoUpdateMainThreadListeners = new HashSet<>();
    ...
    public void addMediaPlayerListener(MediaPlayerWrapper.MainThreadMediaPlayerListener listener) {
        ...
        synchronized (mMediaPlayerMainThreadListeners) {
            if (mMediaPlayerMainThreadListeners.contains(listener)) {
                return;
            }
    ...
    public void addVidoeProgressUpdateListener(MediaPlayerWrapper.VideoStateListener listener) {
        synchronized (mVideoUpdateMainThreadListeners) {
            ...
            mVideoUpdateMainThreadListeners.add(listener);
        }
    }
    ...
    private void notifyVideoPlayTimeUpdateMainThread(int positionInMilliseconds) {
        ...
        for (MediaPlayerWrapper.VideoStateListener listener : listCopy) {
    ...
    private void notifyPreparedMainThread(MediaPlayer mp) {
        if (SHOW_LOGS) Logger.v(TAG, "notifyOnErrorMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners) {
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
    ...
    private void notifyOnInfoThread(MediaPlayer mp, int what, int extra) {
        if (SHOW_LOGS) Logger.v(TAG, "notifyOnInfoThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners) {
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    private void notifyPrepareMainThread() {
        if (SHOW_LOGS) Logger.v(TAG, "notifyPrepareMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners) {
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    private void notifyPausedMainThread() {
    ...
    private void notifyStartedMainThread() {
    ...
    private void notifyOnVideoBufferingUpdateMainThread(int percent) {
        if (SHOW_LOGS) Logger.v(TAG, "notifyOnVideoBufferingUpdateMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners) {
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    public void onBufferingUpdateMainThread(int percent) {
        notifyOnVideoBufferingUpdateMainThread(percent);
    }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        notifyPreparedMainThread(mp);
    }

    @Override
    public void onPrepare() {
        notifyPrepareMainThread();
    }

    @Override
    public void onVideoPausedMainThread() {
        notifyPausedMainThread();
    }

    @Override
    public void onVideoStartedMainThread() {
        notifyStartedMainThread();
    }

    @Override
    public void onInfo(MediaPlayer mp, int what, int extra) {
        notifyOnInfoThread(mp, what, extra);
    }
...
//        if (SHOW_LOGS) Logger.v(TAG, "onSurfaceTextureUpdated, mIsVideoStartedCalled " + mIsVideoStartedCalled.get() + ", mMediaPlayer.getState() " + mMediaPlayer.getState());
    ...
    public void onVideoPlayTimeChanged(int positionInMilliseconds) {
        if (BuildConfig.DEBUG) {
            Log.e(TAG, "onVideoPlayTimeChanged " + positionInMilliseconds);
        }
        notifyVideoPlayTimeUpdateMainThread(positionInMilliseconds);

//        notifyOnVideoProgressUpdateMainThread(positionInMilliseconds);
